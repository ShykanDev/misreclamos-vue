<template>
  ...
  >
    <h3 class="flex gap-2 items-center mb-4 text-xl font-bold"><v-icon name="bi-reply-all-fill" scale="1.2" />Responder Comentario</h3>
    <form @submit.prevent>
      <div class="mb-5">
        ...
        ></textarea>
        <label class="inline-flex gap-2 items-center px-3 py-2 my-2 text-white bg-blue-600 rounded-lg cursor-pointer hover:bg-blue-700">
  <svg
    aria-hidden="true"
    stroke="currentColor"
    stroke-width="2"
...
<img v-if="imageSelected" :src="imageSelected" :key="imageSelected" class="rounded-3xl border-2 border-dashed animate-bounce animate-once w-xs border-slate-500" alt="Imagen seleccionada">
<div>
  <button v-if="imageSelected" @click="removeImage" type="button" class="px-4 py-2 mt-2 text-sm font-medium text-white bg-rose-600 rounded-lg transition-colors cursor-pointer w-sm hover:bg-rose-700">
    Quitar imagen
  </button>
</div>
        <div>
          <p class="text-sm font-medium">¿Es usted la empresa que responde?</p>
          <div class="flex gap-2 items-center mt-2.5 cursor-pointer select-none" @click="setIsCompany(!isCompany)">
            <CheckboxComponent @callSetIsCompany="setIsCompany" @click="setIsCompany(!isCompany)" :checked="isCompany" />
            <small v-show="!isCompany">Marque la casilla si es usted la empresa/servicio que responde</small>
      ...
      <small class="mt-9 text-xs text-slate-400">ⓘ Los comentarios publicados son responsabilidad exclusiva de quien los emite. Nos reservamos el derecho de moderar, eliminar o reportar aquellos que infrinjan nuestras políticas o leyes aplicables.</small>
      <div class="flex justify-center">
        <DotLottieVue v-if="showLottie" style="height: 100px; width: 100px" autoplay src="https://lottie.host/75d6cb4b-837d-4961-8023-98bd5af4c24e/hysY57apaT.lottie" />
        <DotLottieVue v-if="showLottieError && !showLottie"  style="height: 100px; width: 100px" autoplay loop  src="https://lottie.host/89bd3582-e658-4f35-b821-103912eec036/hKwlxD5lVu.lottie" />
      </div>
      <button
      v-if="!showLottie"
      :disabled="isLoading"
...
const answerComment = async () => {
 ...
 if (imageSelected.value) {
    ...
    getAnswerData().image = compressedImage
  }
