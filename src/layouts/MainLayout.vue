<template>
  <header class="sticky top-0 z-50 w-full shadow-sm backdrop-blur-md bg-white/80">
    <section class="flex justify-between items-center px-4 py-3 mx-auto max-w-8xl sm:px-6 lg:px-8">
      <!-- Logo -->
      ...
      <nav class="hidden space-x-8 md:flex">
        ...
        <RouterLink
          :to="{ name: 'all-categories', params: { category: 'Abarrotes y Bebidas' } }"
          class="relative px-3 py-2 text-gray-600 transition-colors hover:text-rose-900 group"
          active-class="font-medium text-rose-900"
        >
          Reclamos
      ...
      >
        <v-icon
          v-if="!mobileMenu"
          name="hi-menu"
          class="animate__animated animate__rubberBand animate-duration-[.4s]"
        />
        <v-icon
          v-else
          name="io-close"
          class="animate__animated animate__rotateIn animate-duration-[.4s]"
        />
      </button>
    </section>
  </header>
  ...
  <main>
    ...
    <article
      class="fixed inset-0 z-40 bg-white transition-transform duration-300 ease-out md:hidden min-h-dvh"
  ...
  <footer class="py-12 text-rose-900 bg-white border-t-4 border-rose-600">
    <div class="container px-4 mx-auto">
      ...
      <div class="grid grid-cols-1 gap-8 mb-12 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="hidden lg:col-span-2 lg:block">
          <p class="text-rose-900 !text-lg">
            ...
            aquí sus experiencias y hagamos saber nuestras inconformidades sobre servicios,
            empresas, productos, personas y más.
          </p>
        </div>
      ...
      <div class="pt-6 text-center text-rose-900 border-t border-rose-800">
        <p>
          misreclamos.com <span id="year"></span> &copy; Todos los derechos reservados
          <span>sitio actualizado por: <span>PACA</span></span>
        </p>
      </div>
    </div>
  </footer>
