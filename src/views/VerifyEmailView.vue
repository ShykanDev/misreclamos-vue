    ...
    <template #main>
      ...
      <div v-else
        ...
        <div
          ...
          <div v-if="isSuccessEmail" class="p-4 text-center text-green-700 bg-green-50 rounded-xl animate-fade-down">
            <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto mb-2 w-8 h-8 text-green-500" fill="none"
              viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
            </svg>
            <p class="font-medium">¡{{ mode === 'verifyEmail' ? 'Correo' : 'Contraseña' }} verificado con éxito!</p>
          ...
          <div v-if="isError || !isValidCode" class="p-4 text-center text-red-700 bg-red-50 rounded-xl animate-fade-up">
            ...
            <p class="font-medium">Error al verificar el {{ mode === 'verifyEmail' ? 'correo' : 'contraseña' }}, código
              de error: {{ errorMessage }}</p>
            <p class="mt-2 text-sm">Por favor, verifique que los parámetros del enlace sean correctos.</p>
          </div>

          <section v-if="mode === 'resetPassword' && !isPasswordVerified"
              ...
              <div class="p-8 bg-white rounded-2xl shadow-xl">
                ...
                <form class="space-y-4">
                  ...
                  <div>
                    ...
                    <input v-model="confirmNewPassword" type="password" id="confirmPassword"
                      class="px-4 py-2 w-full rounded-xl border border-gray-300 transition focus:ring-2 focus:ring-rose-400 focus:border-rose-400"
                      placeholder="••••••••" required />
                  </div>
